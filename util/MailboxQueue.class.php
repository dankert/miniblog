...
class MailboxQueue extends Queue
{
	public $debug = false;
	...
	public function pull()
	{
		...
		$inbox = imap_open($this->hostname,$this->username,$this->password) or die('Cannot connect to IMAP server: ' . imap_last_error());
	...
	{
		$headers = '';
		$headers .= 'Queue-Time: '.time()."\r\n";
		$headers .= 'Queue-User: '.get_current_user()."\r\n";
		$headers .= 'From: '.$this->from. "\r\n";
		$headers .= 'X-Mailer: MailboxQueue/PHP/' . phpversion();
		
		mail($this->receiver, $this->subject, json_encode($entry->value), $headers);		
	}
